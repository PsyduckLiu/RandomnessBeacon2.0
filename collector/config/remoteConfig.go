...
func ReadFileStringArray(filename string) []string {
	file, err := os.Open(filename)
	if err != nil {
		panic(fmt.Errorf("===>[ERROR from ReadFileStringArray]Read file failed:%s", err))
	}
	defer file.Close()
	...
	fmt.Println("==>[ReadFileStringArray] Read ", results)
